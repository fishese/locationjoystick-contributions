...
A route may pin a speed profile via the Route Detail (edit) screen — a segmented control below the name field, showing "None" plus all 5 presets. Default: `null` ("None"), meaning start uses whatever profile is currently active globally. When a route pins a profile, `StartRouteReplayUseCase` calls `SettingsRepository.activateSessionSpeed(route.speedProfileId)` so replay **starts** at that speed and the global active profile (widget icon included) switches to it. After start, live `getActiveSpeedProfile()` changes always update replay (`ReplayOrchestrator.updateSpeed`) — widget Speed Cycle and Settings changes win for the rest of the session. They are not written back into `Route.speedProfileId`. `getRouteSpeedMs` remains for read-only resolution without activating a profile.
