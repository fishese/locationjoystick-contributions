...
| `docs/wiki/changelog.html` + `docs/wiki/changelog/<version>.json` (@docs/features/whats-new.md) | Any release with user-visible changes — write both together: the HTML is the human-readable full changelog, the JSON is the in-app What's New popup's only content source (packed into the APK for offline use, no app code to keep in sync) |
...
| `PositionJitterCoordinator` | `:core:location` | Class (not service) | Instantiated by `MockLocationService`. Owns a persistent position-jitter offset that walks toward a randomly-chosen target inside the configured radius, stepping at most `AppSettings.jitterMaxStepMeters` per tick, instead of teleporting between interval-gated Gaussian draws. |
