        ...
        Column(
            ...
            Column(
                ...
                LjCheckboxRow(
                    ...
                    title = "Swap lat / lon order",
                    description = "Use this when the first number is longitude",
                )
                Row(
                ...
                Text("Point order", style = MaterialTheme.typography.titleSmall)
                LjSegmentedControl(
                    options =
                        listOf(
                            PastePointOrder.ORIGINAL to "Keep original",
                            PastePointOrder.PROXIMITY to "Optimize proximity",
                        ),
                ...
                LjSegmentedControl(
                    options =
                        listOf(
                            PasteBuildMode.AS_IS to "As-is",
                            PasteBuildMode.WALKABLE to "Walkable path",
                            PasteBuildMode.PLANTING to "Planting mode",
                        ),
                ...
                if (state.buildMode == PasteBuildMode.PLANTING) {
                    OutlinedTextField(
                        ...
                        modifier = Modifier.fillMaxWidth(),
                        label = { Text("Circle radius (meters)") },
                        supportingText = {
                            Text(
                                "Default 35 m. Circles stay geometric; only travel between them can follow roads.",
                            )
                        },
                        keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Decimal),
                        singleLine = true,
                    )
                    Text("Between circles", style = MaterialTheme.typography.titleSmall)
                    LjSegmentedControl(
                        options =
                            listOf(
                                PlantingTravel.STRAIGHT to "Straight",
                                PlantingTravel.ROADS to "Via roads",
                            ),
                ...
                LjPrimaryButton(
                    text = if (state.isBuilding) "Building…" else "Build preview",
